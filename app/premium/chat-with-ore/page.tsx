...
const ChatWithOre = () => {
  ...
  return (
    <div>
      <Head>
        <title>Zabbot - Premium: Chat with Ore</title>
        <meta
          name="description"
          content="Chat with Ore on Zabbot Language Learning Platform"
        />
        ...
        <section className="w-full">
          <div className="flex">
            ...
            <div className="hidden lg:block w-1/4 max-w-80 border-r p-2 pr-20 bg-[#002557] text-white" style={{
              ...
              <button onClick={startNewChat} className="w-full bg-blue-500 mb-5 mt-10 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                + New Chat
              </button>

              <hr className="my-5" />

              <div className="h-[calc(100vh-200px)] overflow-y-auto text-white no-scrollbar">
                {conversations.map(item => (
                  <div key={item.id} className="cursor-pointer" onClick={() => switchConversation(item.id)}>
                    <ConversationCard 
                      id={item.id}
                      title={item.title} 
                      date={item.date}
            ...
            {/* Right Pane */}
            {/* Main content area */}
            <div className=" flex overflow-hidden w-full" style={{ fontFamily: "Lexend", backgroundColor: backgroundColor }}>
              ...
              <main className="flex-1 bg-[#04326B] overflow-y-auto p-6 ">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                  {currentConversation.messages.map((msg: any, idx: any) => (
                    <div
                      key={idx}
                ...
                <div className="flex items-center p-4 border border-blue-500 rounded-lg mt-30 w-full">
                  ...
                  <input
                    ...
                    className="bg-white flex-grow mx-2 px-3 h-15 py-1 bg-transparent text-gray-700 placeholder-gray-500 focus:outline-none rounded-lg"
                    value={inputValue}
